...
import type { WhyUsCard } from "@/lib/generated/prisma/client";
import { createWhyUsCard, deleteWhyUsCard, reorderWhyUsCards, updateWhyUsCard } from "@/lib/actions/whyus";
...
export default function WhyUsManager({ initialCards }: { initialCards: WhyUsCard[] }) {
  const { push } = useToast();
  ...
  function openEdit(card: WhyUsCard) {
    setForm({
      id: card.id,
      icon: card.icon,
      title: card.title,
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (form.id) {
        const updated = await updateWhyUsCard(form.id, payload);
        setCards((prev) => prev.map((c) => (c.id === updated.id ? updated : c)));
        push("success", "Kart güncellendi.");
      } else {
        const created = await createWhyUsCard(payload);
        setCards((prev) => [...prev, created]);
        push("success", "Kart eklendi.");
      }
      setModalOpen(false);
    } catch (err) {
  ...
  async function handleReorder(orderedIds: string[]) {
    setCards((prev) => orderedIds.map((id) => prev.find((c) => c.id === id)!).filter(Boolean));
    try {
      await reorderWhyUsCards(orderedIds);
    } catch {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {cards.length === 0 ? (
        <p className="glass rounded-2xl p-6 text-center text-sm text-muted">Henüz kart eklenmedi.</p>
      ) : (
        <SortableList items={cards} getId={(c) => c.id} onReorder={handleReorder}>
          {(card, handle) => {
              ...
              <div className="glass flex items-center gap-3 rounded-2xl p-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="truncate text-sm font-bold text-navy-950 dark:text-white">{card.title}</p>
                    {!card.active && (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={form.id ? "Kartı Düzenle" : "Yeni Kart"}>
